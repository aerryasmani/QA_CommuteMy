...
export async function ValidateDonatePage(page) {
  ...
  await expect(
    page.getByText("Love using this public transport journey planner?"),
  ).toBeVisible();

  const GithubSponsor = page.locator(
    'a:has(svg.lucide-github):has-text("GitHub Sponsor")',
  );
  await expect(GithubSponsor).toBeVisible();

  const KofiSponsor = page.locator(
    'a[href="https://ko-fi.com/zackptr"]:has(svg.lucide-coffee)',
  );
  await expect(KofiSponsor).toBeVisible();

  const Ethereum = page.locator(
    'p:has(svg.lucide-wallet):has-text("Ethereum")',
  );
...
export async function trademark(page) {
  const MalaysianText = page.locator(
    'p:has-text("🇲🇾"):has-text("Built by Malaysian, for Malaysians")',
  );
