...
test.describe("Homepage", () => {
  ...
  test.describe("Pathfinder Section", () => {
    ...
    test("005 | Path Finder field is present and functional", async ({
      page,
    }) => {
      await PathfinderInput(page);
    });

    test("006 | Path Finder swap button button is present and visible", async ({
      page,
    }) => {
      await PathfinderInput(page);
      await PathfinderSwapButton(page);
    ...
    test("007 | Search Route button is present and working", async ({
      page,
    }) => {
      await PathfinderInput(page);
    ...
    test("008 | Bottom text and github issue link is present", async ({
      page,
    }) => {
  ...
  test.describe("Browse Line Section", () => {
    test("009 | Browse Line Section is present and visible", async ({
      page,
    }) => {
      await LinesButton(page);
    });

    test("010 | Validate the functionality of lines button", async ({
      page,
    }) => {
